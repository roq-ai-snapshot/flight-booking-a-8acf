...
import { useRoqClient, useCustomerServiceFindFirst } from 'lib/roq';
...
function CustomerServiceEditPage() {
  ...
  const { data, error, isLoading, mutate } = useCustomerServiceFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: CustomerServiceInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.customer_service.update({
        data: values as RoqTypes.customer_service,
        where: {
          id,
        },
      });
